...
class RemainingTimeCalculator {
    ...
    public void setStoragePath(int path, String extPath) {
        if (extPath != null) {
...
public class SoundRecorder extends Activity
    ...
    static final int FOCUS_CHANGE = 0;
    ...
    private static boolean BLUETOOTH_RECORDING_ENABLED = true;
    private BluetoothAdapter mBluetoothAdapter;
    private BluetoothHeadset mBluetoothHeadset;
    private int mBluetoothScoState = -1;
    private boolean mBluetoothScoOwned;
    ...
    private boolean mStartRecordingWithBtPending;
    private static final int BLUETOOTH_SCO_MODE_TIMEOUT = 5000;
    ...
    public void onCreate(Bundle icycle) {
        ...
        registerReceiver(new BroadcastReceiver() {
            ...
            public void onReceive(Context context, Intent intent) {
                ...
                Log.d(TAG, "Bluetooth SCO State Change: " + state);
                ...
                switch (msg.what) {
                    ...
                    case START_RECORDING:
                        if (BLUETOOTH_RECORDING_ENABLED) {
                            if (mAudioSourceType == MediaRecorder.AudioSource.MIC && startBluetoothAsync()) {
                                // we just started an async bluetooth connection request
                                // save the arguments and invoke START_RECORDING_WITH_BT later
                                mBluetoothScoPendingRecordArg1 = msg.arg1;
                                mBluetoothScoPendingRecordArg2 = msg.arg2;
                                Log.d(TAG, "bt starting async");
                                break;
                            }
                        }
                        // fall-thru
                    ...
                    case STOP:
                        ...
                        if (mBluetoothScoOwned) {
                            Log.d(TAG, "Stopping Bluetooth SCO");
                            mAudioManager.stopBluetoothSco();
                            mBluetoothScoOwned = false;
                        }
                        break;
                    case STOP_AND_SAVE:
                        mRecorder.stop();
                    ...
                    case BLUETOOTH_SCO_MODE_TIMEDOUT:
                        ...
                        mStartRecordingWithBtPending = false;
                        mBluetoothScoOwned = false;
                        mAudioManager.stopBluetoothSco();
                        // TODO
                        Toast.makeText(SoundRecorder.this, "Bluetooth timeout", Toast.LENGTH_SHORT).show();
                        break;
    ...
    private boolean startBluetoothAsync() {
        ...
        for (final BluetoothDevice dev : devices ) {
            if (mBluetoothHeadset.isAudioConnected(dev)) {
                // TODO
                Toast.makeText(this, "Using bluetooth headset", Toast.LENGTH_SHORT).show();
                Log.d(TAG, "headset " + dev.getName() + " audio connected " + dev.getAliasName());
                return false;
            }
        }

        Log.d(TAG, "Starting SCO Mode");
        // TODO
        mBluetoothConnectingToast = Toast.makeText(this, "Connecting to headset", Toast.LENGTH_SHORT);
        mBluetoothConnectingToast.show();

        mStartRecordingWithBtPending = true;
        mBluetoothScoOwned = true;
        mAudioManager.startBluetoothSco();
        mRecordHandler.sendEmptyMessageDelayed(BLUETOOTH_SCO_MODE_TIMEDOUT, BLUETOOTH_SCO_MODE_TIMEOUT);
        return true;
    }
    ...
    private void onBluetoothScoStateChanged(int state) {
        ...
        switch (state) {
            ...
            case AudioManager.SCO_AUDIO_STATE_DISCONNECTED:
                if (mBluetoothScoState == AudioManager.SCO_AUDIO_STATE_CONNECTED) {
        ...
        if (mBluetoothScoConnected) {
            if (!mStartRecordingWithBtPending) {
                return;
            }
            mStartRecordingWithBtPending = false;
    ...
    private void recordButtonAction() {
        if (!hasAllPermissions()) {
            ...
            if (mPath == 1 &&
                ...
                /* we need to fall back to phone storage if external storage not mounted
                   since the menu to set it back to phone storage is removed.
                 */
                ...
                updateUi();
                Toast.makeText(this, R.string.storage_fallback, Toast.LENGTH_SHORT).show();
            }
            mRemainingTimeCalculator.reset();
            mRemainingTimeCalculator.setStoragePath(mPath, getSDPath());
            mRecorder.setStoragePath(mStoragePath + "/SoundRecorder");
            if (mPath == 0 &&
    ...
    class StorageData {
        ...
        public StorageData(String name, String path) {
            mName = name;
            mPath = path;
        }
    }

    private int buildStorage(ArrayAdapter<StorageData> adapter) {
        StorageManager sm = (StorageManager) SoundRecorder.this.getSystemService(Context.STORAGE_SERVICE);
        StorageVolume[] volumes = sm.getVolumeList();
        int def = 0;

        adapter.add(new StorageData(getResources().getString(R.string.storage_setting_local_item)
                        , STORAGE_PATH_LOCAL_PHONE));

        for (int i = 0, j = 0; i < volumes.length; i++) {
            ...
            if (sm.getVolumeState(v.getPath())
                ...
                adapter.add(new StorageData(v.getDescription(SoundRecorder.this), v.getPath()));
                if (v.getPath().equals(mStoragePath)) {
                    def = j + 1;
                }
                j++;
            }
    ...
    private void openStorageOptionDialog() {
        ...
        final ArrayAdapter<StorageData> adapter = new ArrayAdapter<StorageData>(this,
            ...
            public View getView(int position, View convertView, ViewGroup parent) {
                ...
                final StorageData i = this.getItem(position);
                ((TextView)convertView).setText(i.mName);
                return convertView;
            }
        };
        int selectedItem = buildStorage(adapter);

        final DialogInterface.OnClickListener clickListener =
                new DialogInterface.OnClickListener() {
            public void onClick(DialogInterface dialog, int which) {
                dialog.dismiss();
                final StorageData res = adapter.getItem(which);
                if (which == 0) {
                    /* This is the internal phone storage */
                    mStoragePath = STORAGE_PATH_LOCAL_PHONE;
                    mPath = 0;
                    mPrefsStoragePathEditor.putString("storagePath", mStoragePath);
                    mPrefsStoragePathEditor.putInt("path", mPath);
        ...
        ad = new AlertDialog.Builder(this)
            ...
            .setSingleChoiceItems(adapter, selectedItem, clickListener)
            .create();
        ad.setCanceledOnTouchOutside(true);
        ad.show();
    }

    private void openFileTypeOptionDialog() {
        final Context dialogContext = new ContextThemeWrapper(this,
                android.R.style.Theme_Material_Dialog);
        final Resources res = dialogContext.getResources();
        final LayoutInflater dialogInflater = (LayoutInflater) dialogContext
    ...
    public boolean onPrepareOptionsMenu(Menu menu) {
        ...
        if (extStorage) {
            menu.findItem(R.id.menu_item_storage).setVisible(true);
            menu.findItem(R.id.menu_item_storage).setEnabled(mRecorder.state() == Recorder.IDLE_STATE);
        } else {
            menu.findItem(R.id.menu_item_storage).setVisible(false);
        }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            ...
            case R.id.menu_item_filetype:
                if(mRecorder.state() == Recorder.IDLE_STATE) {
                    openFileTypeOptionDialog();
                }
                break;
            case R.id.menu_item_storage:
                if(mRecorder.state() == Recorder.IDLE_STATE) {
                    openStorageOptionDialog();
                }
                break;
    ...
    private String getSDPath() {
        ...
        String storagePath = mSharedPreferences.getString("storagePath", null);
        return storagePath;
    }
